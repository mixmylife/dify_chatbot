...
import MeditechLogo from '@/app/components/base/logo/meditech-logo'
...
const Header: FC<IHeaderProps> = ({
  ...
  if (!isMobile) {
    return (
      <div className='flex h-30 shrink-0 items-center justify-end p-10'>
              ...
              <div className={cn(
                ...
                <div className='system-2xs-medium-uppercase text-text-tertiary'>{t('share.chat.poweredBy')}</div>
                {
                  systemFeatures.branding.enabled && systemFeatures.branding.workspace_logo
                    ? <img src={systemFeatures.branding.workspace_logo} alt='logo' className='block h-30 w-auto' />
                    : appData?.custom_config?.replace_webapp_logo
                      ? <img src={`${appData?.custom_config?.replace_webapp_logo}`} alt='logo' className='block h-30 w-auto' />
                      : <MeditechLogo size='large' className="!w-66 !h-30" />
                }
              </div>
            )}
          </div>
